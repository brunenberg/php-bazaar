...
<div class="mb-5">
    <form action="{{ route('search') }}" method="GET" class="w-full flex justify-center">
        ...
        <select name="sort_by" class="ml-3 border-2 border-gray-300 p-2 rounded-xl">
            <option value="updated_at_desc">{{ __('Sort by Latest') }}</option>
            <option value="updated_at_asc">{{ __('Sort by Oldest') }}</option>
            <option value="price_asc">{{ __('Sort by Price: Low to High') }}</option>
            <option value="price_desc">{{ __('Sort by Price: High to Low') }}</option>
        </select>
        <input type="text" name="min_price" class="ml-3 w-1/6 border-2 border-gray-300 p-2 rounded-xl" placeholder="{{ __('Min Price') }}" value="{{ request('min_price') }}">
        <input type="text" name="max_price" class="ml-3 w-1/6 border-2 border-gray-300 p-2 rounded-xl" placeholder="{{ __('Max Price') }}" value="{{ request('max_price') }}">
