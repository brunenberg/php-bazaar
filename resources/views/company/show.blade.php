...
@if (auth()->check() && auth()->user()->user_type === 'admin')
    <div class="mx-20 mt-5 p-5 rounded bg-gray-200">
        <h2 class="font-bold text-2xl pb-3">{!!__('Download new contract')!!}</h2>
        <form action="{{route('company/download-contract')}}" method="POST">
            @csrf
            <input type="hidden" name="company_id" value="{{$company->id}}">
            <button type="submit" id="downloadContract" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">{!!__('Download')!!}</button>
        </form>
        ...
        <form action="{{route('company/upload-contract')}}" method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-2">{!!__('Upload new contract')!!}</button>
        </form>

        {{-- Show all contracts uploaden and if they are signed in a table--}}
        <h2 class="font-bold text-2xl pt-5 pb-3">{!!__('Contracts')!!}</h2>
        <table class="w-full">
            <thead>
                <tr>
        ...
        <a href="/companies" class="mt-2 text-blue-700 hover:underline">{!!__('Back to companies')!!}</a>
    </div>
@endif
